...
main().catch(e => (0, core_1.setFailed)(e.message));
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        const token = (0, core_1.getInput)('token', { required: true });
        const projectId = (0, core_1.getInput)('project_id', { required: true });
        const staging = ((0, core_1.getInput)('staging', { required: false }) || 'false').toLowerCase() === 'true';
        const apiUrl = staging ? STAGING_API_URL : API_URL;
        const versionsResponse = yield (0, node_fetch_1.default)(`${apiUrl}/project/${projectId}/version`);
        if (versionsResponse.status !== 200) {
            (0, core_1.setFailed)(`Failed to get versions of project with id ${projectId}`);
            return;
        }
        const versions = (yield versionsResponse.json());
        const allGameVersions = [
            ...new Set([].concat(...versions.map(v => v.game_versions))),
        ];
        const shouldBeFeatured = [];
        for (const gameVersion of allGameVersions) {
            const newest = versions
                .filter(v => v.game_versions.includes(gameVersion))
                .reduce((prev, current) => Date.parse(prev.date_published) >
                Date.parse(current.date_published)
                ? prev
                : current);
            if (!shouldBeFeatured.includes(newest))
                shouldBeFeatured.push(newest);
        }
        for (const version of versions) {
            ...
            if (featured === null)
                continue;
            yield (0, node_fetch_1.default)(`${apiUrl}/version/${version.id}`, {
                method: 'PATCH',
                headers: {
